...
const SHORTCUTS = [
  { href: "/admin/announcements", label: "공지관리" },
  { href: "/admin/photos", label: "사진관리" },
  { href: "/admin/surveys", label: "설문목록" },
];
...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {data.ok && (
        <>
          <section aria-label="설문 응답 요약" className="grid grid-cols-2 gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-amber-100 bg-white px-5 py-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <p className="text-xs text-zinc-500 dark:text-zinc-400">전체 설문 응답 수</p>
              <p className="mt-1 text-2xl font-bold text-zinc-900 dark:text-zinc-50">
                {data.responses.length}건
              </p>
            </div>
            <div className="rounded-xl border border-amber-100 bg-white px-5 py-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
              <p className="text-xs text-zinc-500 dark:text-zinc-400">고유 참가자 수</p>
              <p className="mt-1 text-2xl font-bold text-zinc-900 dark:text-zinc-50">
                {new Set(data.responses.map((r) => r.participantName)).size}명
              </p>
            </div>
          </section>
          ...
          <section aria-labelledby="recent-heading" className="flex flex-col gap-3">
            ...
            {data.responses.length === 0 ? (
              <p className="text-sm text-zinc-500 dark:text-zinc-400">아직 제출된 설문 응답이 없습니다.</p>
            ) : (
      ...
      <section aria-label="바로가기" className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {SHORTCUTS.map((s) => (
          <Link
            key={s.href}
            href={s.href}
